...
H3K4me3 shows 70 regions significantly enriched for $FDR=0$. The "T.filtered"
regions are not considered for multiple testing correction because the counts
therein fall below a computed threshold (Dialsingh *et al.*, Bioinformatics,
2015, 1–7). This quantity is very large in our example because the toy alignment
files are filtered for reads within chr1 22.5Mb to 25Mb. However, the specified
genome covers chr1:0-249250621 which results in alot of zero counts. The summary
for H3K27me3 enrichment calls confirms 4831 regions significantly enriched for
$FDR<=0.001$:
...
In addition to summary statistics, `NormRFit` object can be plotted.



Further processing of regions within R can be facilitated by getting
significantly enriched regions as a `GRanges` object.
...
exportR(k4me3Fit, filename = "k4me3Fit.bw", type = "bw")
```
