...
#require(parallel)
#require(diffr)
#diffR <- function(s, r, eps=.0001){
#  idx <- 1:length(s) #which(s > 0 & r > 0)
#  N = length(idx)
#  nmodels = 3
#  #transitions <- runif(nmodels)
#  transitions <- c(0.9, 0.06, 0.04)
#  theta <- c(0.3, 0.6,0.9) #rep(sum(s[idx]) / sum(r[idx] + s[idx])) + runif(nmodels, 0, .1) # q*
#  theta = sort(theta)
#  s = s[idx]
#  r = r[idx]
#  n = s + r
...
#    lnZ = diffr:::logRowSum(likelihood)
...
#}
#
#cppFunction("double cdbinom(double k, double n, double p, int log) { return R::dbinom(k,n,p,log); }")
#
