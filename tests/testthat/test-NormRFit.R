...
test_that("Test NormRFit construction, validity and accessors", {
  ...
  for(type in c("enrichR", "diffR", "regimeR")) {
    ...
    o <- new("NormRFit", type=type, n=as.integer(n),
             ...
             counts=list(d$map$values[2,],d$map$values[1,]), 
    ...
    enr <- d$lnenr; if (o@type != "diffR") enr <- exp(enr)
      expect_equal(getEnrichment(o), enr)
    expect_equal(getPosterior(o), exp(d$lnpost))
