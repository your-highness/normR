...
In addition to summary statistics, a `NormRFit` object can be plotted.

TODO:plotting

Further processing of regions within R can be facilitated by getting
significantly enriched ($FDR \le 0.05%) regions as a `GRanges` object.
...
As an illustrative analysis, we would like check for overrepresentation of
H3K4me3-enriched regions with genes and promoters by using Fisher's exact test.
...
joint analysis of H3K4me3 and H3K27me3 ChIP-seq data. Because we already counted
`k4me3Bamfile` and `k27me3Bamfile` already in `k4me3Fit` and `k27me3Fit`,
...
strategy. However, the "Number of Components" is now 3 representing (i) H3K27me3
enrichment without H3K4me3-enrichment, (ii) H3K27me3 and H3K4me3 (non)-enriched
and (iii) H3K4me3 enrichment without H3K27me3-enrichment. The
...
"Mixture Proportions", regions are classified to (i) in 75%, (ii) in 20% and
(iii) in 5% of the regions.

4752 (4645+52+55) regions are significantly different from background with $FDR
\le 0.05$.

H3K4me3 shows 577 (70+389+57+61) regions significantly enriched for $FDR \le
0.05$. The "T.filtered" regions are not considered for multiple testing
correction because the counts therein fall below a computed threshold (Dialsingh
*et al.*, Bioinformatics, 2015, 1–7). This quantity is very large in our example
because the toy alignment files are filtered for reads within chr1 22.5Mb to
25Mb. However, the specified genome covers chr1:0-249250621 which results in
alot of zero counts. The summary for H3K27me3 enrichment calls confirms 5493
(0+4831+377+285) regions significantly enriched for $FDR \le 0.05$:
...
In addition to the information covered above and in the manpages (`?`) of normR
functions, we would like to discuss some important points here:
