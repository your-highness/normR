...
statistics and diagnostic plots are helpful for studying results. In a latter
section, we cover more advanced topics including the alteration of read counting
strategies, the application of normR on user-defined regions (*e.g.*
...
enriched for H3K4me3 mostly at promoters and precludes H3K36me3 enrichment which
is overrepresented in intra-genic regions.
...
H3K4me3 and H3K36me3 ChIP-seq) containing reads for human chr1 22.5Mb to
25Mb. Note, bam files need to be sorted by read coordinates (`samtools sort
x.bam x_sorted`) and indexed (`samtools index x_sorted.bam`). 
...
That was easy and fast! You must know that all results are stored as `NormRFit`
objects. They provide convenient access to count data and fitting results. For
example, let's have a look at some simple fitting statistics for H3K4me3:
...
actual $\theta_B$ is with $0.15$ much smaller than $\theta^*$ which allows for
more sensitive enrichment calling.  Furthermore, by looking at the "Mixture
Proportions" we find H3K4me3 is enriched in 6% of the windows. For H3K36me3, we
find 28% of the regions enriched.
...
statistical testing. The "T-Filter threshold" defines regions that are not
considered for multiple testing correction where the sum of counts in treatment
and control is less than this quantity (Dialsingh *et al.*, Bioinformatics,
2015, 1–7). The "Number of Regions filtered out" is very large in our example
because the toy alignment files are filtered for reads within chr1 22.5Mb to
25Mb. However, the specified genome covers chr1:0-249250621 which results in
alot of zero counts. This does not influence the fit but for testing the regions
are filtered. Based on computed q-vlaues, H3K4me3 shows 577 (70+389+57+61)
regions significantly enriched for $FDR \le 0.05$. The summary for H3K36me3
enrichment calls confirms 2332 (0+1994+170+168) regions significantly enriched
for $FDR \le 0.05$:
...
In addition to summary statistics, a `NormRFit` object can be plotted with a
specialized implementation of `plot()`:
```{r}
#TODO
```
...
recover the regions signficantly enriched in `k4me3Fit` and `k36me3Fit`.
...
significantly overrepresented within genes and, more pronounced (higher odds),
in promoter regions. By comparing H3K36me3 and H3K4me3 enrichment, we can
identify significant overlap of H3K36me3 and H3K4me3 that is occuring with
higher odds in promoter regions than compared to gene bodies.
...
presence of H3K36me3 within the gene body marks expressed genes.  Actually,
H3K36me3 enrichment is significantly overrepresented at genes and promoters.
...
![](../inst/extdata/1b.png)
**IGV browser shot of Input (grey), H3K4me3 (green) and H3K36me3 (purple)
alignment data (bars), normalized enrichment (lines) and enriched regions (boxes
below respective tracks).**
...
conditional changes of ChIP-seq enrichment. Below, we exemplify this analysis by
joint analysis of H3K4me3 and H3K36me3 ChIP-seq data. Because we already counted
`k4me3Bamfile` and `k36me3Bamfile` already in `k4me3Fit` and `k36me3Fit`,
respectively, we can use these counts directly. Note that, in this case, the
`genome` has be set to a `GenomicRanges` object specifying the genomic regions.
We can extract this from either one of the `NormRFit` objects.
...
The "Type" of the object has changed to 'diffR' because it was generated by this
function. The "Number of Regions" did not change because we use the same binning
strategy. However, the "Number of Components" is now 3 representing (i) H3K36me3
enrichment without H3K4me3-enrichment, (ii) H3K36me3 and H3K4me3 (non)-enriched
and (iii) H3K4me3 enrichment without H3K36me3-enrichment. The
...
"Mixture Proportions", regions are classified to (i) in 31%, (ii) in 60% and
(iii) in 9% of the regions. 2164 (0+2096+37+31) regions are significantly
...
![](../inst/extdata/2.png)
**IGV browser shot of Input (grey), H3K4me3 (green) and H3K36me3 (purple)
alignment data. Background normalized difference is plotted as a heatmap and
differential regions are plotted as boxes (blue: treatment (H3K4me3) enriched,
red: control (H3K36me3) enriched).**
...
extendable normR approach also allows for identification of different enrichment
regimes with `regimeR()` by increasing the number of model components.
...
Exporting works analogously to the cases described above. 
...
`BamCountConfig-class` provides methods to define the counting strategy on
...
Copy Number Variations (CNVs) are an important feature of cancerous cells like patient
tumour samples or cell lines. The difference calling on two ChIP-seq experiments
with `diffR()` is sensitive to CNVs if the underlying sequence is amplified in
the genome. However, you can harness `diffR()`'s functionality to call
differences in two Input tracks to detect CNVs in a treatment respective to a
control:
