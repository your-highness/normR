...
* `regimeR()` calls `k` enrichment regimes in a ChIP-seq experiment over control
```{r eval=FALSE}
...
stratification of enrichment regimes. Herein, we provide examples for the export
of results to common data formats like bigWig and bed. We show how analysis
...
strategies, the application of normR on user-defined regions (*e.g.* promoters)
and the integration of Copy Number Variation information in differential
ChIP-seq enrichment calls.
...
is overrepresented in gene bodies.
...
25Mb. Note, bam files used in normR need to be sorted by read coordinates
...
Proportions" we find H3K4me3 is enriched in 5% of the windows. For H3K36me3, we
find 22% of the regions enriched.
...
chr1 22.5Mb to 25Mb. However, the specified genome covers chr1:0-249250621 which
results in alot of zero counts. This does not influence the fit but for testing
the regions are filtered. Based on computed q-vlaues, H3K4me3 shows 380
(49+234+41+56) regions significantly enriched for $FDR \le 0.05$. The summary
for H3K36me3 enrichment calls confirms 1324 (0+1127+110+87) regions
...
significantly overrepresented within genes (Fisher's signed-exact test; P<0.001;
odds ratio = 6.01) and, more pronounced, in promoter regions (odds ratio =
14.74).

By comparing H3K36me3 and H3K4me3 ranges, we can identify significant overlap of
H3K36me3 and H3K4me3 that most pronounced in promoter regions (odds ratio =
27.75) than compared to gene bodies (odds ratio = 8.17).
...
H3K36me3 enrichment is significantly overrepresented mostly at genes (odds ratio
= 19.12) and, to a lower extend, at promoters (odds ratio = 8.19).
...
![](../inst/extdata/1b.png)
**IGV browser shot of Input (grey), H3K4me3 (green) and H3K36me3 (purple)
alignment data (bars), normalized enrichment, _i.e._ "bigWig" files, (lines) and
enriched regions, _i.e._ "bed" files (boxes below respective tracks).**
...
conditional changes in ChIP-seq enrichment. Below, we exemplify this analysis by
...
"Mixture Proportions", regions are classified to (i) in 44%, (ii) in 37% and
(iii) in 19% of the regions. 1,090 (0+1047+22+21) regions are significantly
...
![](../inst/extdata/2.png)
**IGV browser shot of Input (grey), H3K4me3 (green) and H3K36me3 (purple)
alignment data. Background normalized difference is plotted as a heatmap, _i.e._
"bigWig" file, and differential regions are plotted as boxes, _i.e._ "bed" file
(blue: treatment (H3K4me3) enriched, red: control (H3K36me3) enriched).**
...
12% of the regions show enrichment which gets segmented into 9% low and
3% high enrichment. 478 (50+272+63+93) regions are significant ($FDR \le 0.05$).
...
We can now export the called regimes as bed files for browser display. This is
done analogously to the cases described above:
...
tumour samples. The difference calling on two ChIP-seq
experiments with `diffR()` is sensitive to CNVs if the underlying sequence is
amplified in the genome. However, you can harness `diffR()`'s functionality to
call differences in two Input tracks to detect CNVs in a treatment respective to
a control. To allow for coarse-grained detection of difference in Input, a
sufficiently large binsize has to be used, _e.g._ 22kb.
