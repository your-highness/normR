...
k36me3Fit <- enrichR(treatment = k36me3Bamfile, control = inputBamfile, genome = genome, verbose = F)
...
plot(d.x$x, d.x$y, xlim=limits, type='l', main="H3K36me3 normalized Enrichment", xlab="", ylab="Density")
abline(v=0, lty=3, lwd=2, col=4)
plot(d.y$y, d.y$x, ylim=limits, xlim=rev(range(d.y$y)), type='l', main="H3K4me3 normalized Enrichment", xlab="Density", ylab="")
abline(h=0, lty=3, lwd=2, col=4)
plot(x, y, xlim=limits, ylim=limits, col=adjustcolor("grey10", alpha.f=.2), pch=20, xlab="", ylab="")
